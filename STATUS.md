...
> 작성: 2026-05-17 / 기준 브랜치: `dev` / 최근 commit `6c01f39` (scan_mode 데모 실동작 + 가드)
...
- 매핑 학술 검증: `docker exec zt-assessment-zt-backend-1 python /app/scripts/validate_checklist_mapping.py` — 자동 212 ↔ 매핑 212, 누락 0, 잘못 0, 충돌 0, 다중 매핑(의도된 카테고리 다중) 54건
- **진단 실행 모드 (2026-05-17 신설)**: NewAssessment 토글 `scan_mode = demo | live`
  · **demo**: collector 실호출 없이 fake 결과 자동 생성 (충족 60% / 부분충족 25% / 미충족 15%, item_id 해시 기반 deterministic) → 자동 채점 → 시연 즉시 의미 있는 점수·차트·권고 표시 (e2e 검증: overall 2.81 / 향상)
  · **live**: 실제 외부 시스템 호출. 선택된 도구의 자격이 비어있으면 `400` 가드.
- **finalize 가드 (2026-05-17 신설)**: 수집 미완료 상태에서 finalize 호출 시 `409` (`"수집 진행 중입니다 (N/M)"`).
...
│   │   ├── assessment.py             (3086줄) 진단 실행/상태/결과/이력/finalize/webhook/internal collect
│   │   │                              /compare /share/{id} /shared/{token} /session/{id}(DELETE)
│   │   │                              14 도구 dispatcher + scan_mode demo/live 분기 + 자격 메모리 dict + IDOR 차단
...
│   │   └── validators.py             (271줄) 입력 검증 (Nmap/Trivy/URL/자격/Entra GUID/Okta/LDAP/AWS/Azure/CF)
│   │
│   ├── scoring/
...
│   │   ├── aws_security_hub_collector.py (617줄) 15 함수, 클라우드 자세 — AWS Security Hub findings + boto3
│   │   ├── azure_defender_cloud_collector.py (626줄) 15 함수, 클라우드 자세 — Defender for Cloud (ARM REST)
│   │   ├── zscaler_collector.py        (388줄)  10 함수, ZTNA — Zscaler Internet/Private Access API
│   │   ├── cloudflare_access_collector.py (453줄) 10 함수, ZTNA — Cloudflare Zero Trust API
...
| IdP fallback | KEYCLOAK_URL/ADMIN, ENTRA_TENANT_ID/CLIENT_ID/SECRET, OKTA_DOMAIN/API_TOKEN, LDAP_URL/BIND_DN/PASSWORD/BASE_DN | live 모드 자격 fallback |
...
| Cloud Posture | AWS_ACCESS_KEY_ID/SECRET_ACCESS_KEY/REGION, AZURE_TENANT_ID/CLIENT_ID/SECRET/SUBSCRIPTION_ID | |
| ZTNA | ZSCALER_BASE_URL/API_KEY, CLOUDFLARE_API_TOKEN/ACCOUNT_ID | |
| Nmap/Trivy | NMAP_WRAPPER_URL, TRIVY_WRAPPER_URL, NMAP_TARGET, TRIVY_TARGET | |
| Shuffle | SHUFFLE_URL, SHUFFLE_API_KEY, SHUFFLE_ADMIN_USER/PASS/EMAIL, SHUFFLE_WORKFLOW_* (14개) | bootstrap.sh 자동 |

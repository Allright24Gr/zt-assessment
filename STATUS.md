...
사용자 → Frontend(React) → Backend(FastAPI) → MySQL
                          ↓ (옵션) Shuffle SOAR
                          ↓
                  10개 도구 collector → 외부 시스템(Keycloak/Wazuh/Nmap/Trivy/Entra/Okta/Splunk/LDAP/CrowdStrike/Defender)
```

- 진단 기준: **제로트러스트 가이드라인 2.0** — 6 Pillar(신원/기기/네트워크/시스템/애플리케이션/데이터) × 4 성숙도(기존/초기/향상/최적화)
- 자동 진단 항목: **193 base + 114 autodiscover = 307** (10 도구)
...
| wazuh | 41 | 81 | **122** | SIEM | API user/pass → JWT |
...
```
✅ AST: backend 21 파일 + tests 8 파일 모두 통과
✅ _full_mapping() base 매핑:
     keycloak 32 + wazuh 41 + nmap 14 + trivy 11 + entra 20 + okta 15 + splunk 15
     + ldap 15 + crowdstrike 15 + defender 15 = 193 (autodiscover 합산 307)
✅ maturity 일관성: 10 도구 0건 불일치 (_validate_mapping 가드 적용)
✅ _resolve_supported_tools 7+ 케이스 통과 (idp/siem/edr 모든 조합)
✅ frontend npm run build 통과 (chunk-size 경고만, 변경 코드 무관)
✅ validators: shell metachar 차단 (Nmap target/Trivy image/URL/Entra GUID/Okta domain/LDAP URL+DN)
✅ _mask_creds 8 도구 자격 비밀번호 모두 "***" 마스킹

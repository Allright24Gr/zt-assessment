...
> 작성: 2026-05-18 / 기준 브랜치: `dev` = `master` 동기화 (`962725a`)
> 4 오픈소스 도구로 환원 (학생 프로젝트 — 라이선스 비용 0). InProgress 데모 시뮬레이션 보강.
...
사용자 → Frontend(React) → Backend(FastAPI) → MySQL
                          ↓ (옵션) Shuffle SOAR
                          ↓
                  4개 오픈소스 collector → 외부 시스템(Keycloak / Wazuh / Nmap / Trivy)
```

- 진단 기준: **제로트러스트 가이드라인 2.0** — 6 Pillar(신원/기기/네트워크/시스템/애플리케이션/데이터) × 4 성숙도(기존/초기/향상/최적화)
- 자동 진단 항목: **98 base + 114 autodiscover = 212** (4 오픈소스 도구, xlsx 자동 212항목과 1:1 정합 검증 완료)
  · Keycloak 65 + Wazuh 122 + Nmap 14 + Trivy 11 = **212 collector 함수**
- 매핑 학술 검증: `docker exec zt-assessment-zt-backend-1 python /app/scripts/validate_checklist_mapping.py` — 자동 212 ↔ 매핑 212, 누락 0, 잘못 0, 충돌 0
- **운영 도구 4 오픈소스만**: Keycloak (IdP, Apache 2.0) / Wazuh (SIEM·HIDS, GPL v2) / Nmap (외부 스캔, GPL v2) / Trivy (이미지 스캔, Apache 2.0). 학생 프로젝트 — 상용 SaaS(Entra/Okta/Splunk/CrowdStrike/Defender/AWS SH/Azure Defender/Zscaler/Cloudflare) 라이선스 부담 회피.
...
│                                       # --profile shuffle 자동 포함 — 한 번에 모든 컨테이너 기동
...
│   │   ├── assessment.py             진단 실행/상태/결과/이력/finalize/webhook/internal collect
│   │   │                              /compare /share/{id} /shared/{token} /session/{id}(DELETE)
│   │   │                              4 도구 dispatcher + scan_mode demo/live 분기 + 자격 메모리 dict + IDOR 차단
...
│   │   ├── test_collector_mapping.py # 14 cases — 14 도구 매핑 정합성
...
│       ├── types/api.ts              # 235줄+ 백엔드 응답·요청 TS 타입 (14 도구 자격/profile_select/scan_mode)
│       │
│       └── app/
...
[6] POST /api/assessment/run  (Authorization: Bearer)  body: {..., scan_mode}
              ...
              ├ _store_session_secrets ← 자격 메모리 dict (14 도구)
...
[7] _run_collectors (단일 _collector_lock 직렬화)
              ...
              │   ├ 14 도구 매핑 순회 → 항목별 _demo_result 생성 (충족 60%/부분 25%/미충족 15%)
              │   │     · item_id 해시 기반 deterministic — 같은 항목은 항상 같은 결과
              │   │     · DEMO_DELAY_MS 점진 진행률 효과 (예: 220ms × 212 ≈ 47초)
...
## 5. 자동 진단 매핑 (4 오픈소스 도구, xlsx 자동 212항목과 1:1 정합)

| 도구 | base | autodiscover | 합계 | 카테고리 | 인증 방식 | 라이선스 |
|---|---|---|---|---|---|---|
| keycloak | 32 | 33 | **65** | IdP | admin REST API token | Apache 2.0 |
| wazuh | 41 | 81 | **122** | SIEM/HIDS | API user/pass → JWT | GPL v2 |
| nmap | 14 | 0 | **14** | 외부 스캔 | (도구 무관) | GPL v2 |
| trivy | 11 | 0 | **11** | 이미지 스캔 | (도구 무관) | Apache 2.0 |
| **합계** | **98** | **114** | **212** | | | |

> 학술 검증: xlsx 자동 진단 212개 = 우리 매핑 unique item_id 212개. 1:1 정합 — 다중 매핑 0건, 잘못된 매핑 0건, 수동 충돌 0건.

> **왜 4개만**: 학생 프로젝트로 시작 — 라이선스 비용 0의 오픈소스 도구만 실제 검증 가능. 상용 SaaS(Entra/Okta/Splunk/CrowdStrike/Defender/AWS SH/Azure Defender/Zscaler/Cloudflare)는 무료 평가판 + 구독료가 필요해 학생이 부담할 수 없음 → 도입 결정 후 추가 통합 검토.
...
| Shuffle | SHUFFLE_URL, SHUFFLE_API_KEY, SHUFFLE_ADMIN_USER/PASS/EMAIL, SHUFFLE_WORKFLOW_* (4개) | bootstrap.sh 자동 |
...
## 10. 자기 검증 (정적, 2026-05-18 기준)

```
✅ AST: backend 모든 파일 + tests 6 파일 통과
✅ _full_mapping() base 매핑:
     keycloak 32 + wazuh 41 + nmap 14 + trivy 11 = **98** base
     + autodiscover (keycloak 33 + wazuh 81 = 114)
     = **212** 합계 (unique item_id 212)
✅ xlsx ↔ 매핑 학술 검증 (validate_checklist_mapping.py):
     xlsx 자동 212 = 매핑 unique 212 / 잘못된 매핑 0 / 수동 충돌 0 / 다중 매핑 0 (1:1)
✅ maturity 일관성: 4 도구 0건 불일치 (_validate_mapping 가드 적용)
...
✅ scan_mode e2e (2026-05-17, session 31):
     POST /run scan_mode=demo → 212/212 진행률 자연스럽게 차오름 → 자동 채점
     overall_score 2.81 (level=향상) / 6 pillar 모두 점수 정상
     충족 144건, 미충족 68건 (item_id 해시 기반 deterministic)
✅ scan_mode=live + 자격 누락 → 400 "누락: keycloak, wazuh"
✅ finalize 수집 중 호출 → 409 "수집 진행 중입니다 (5/212)"
...
✅ deploy.sh 배포 자동화 (2026-05-18):
   ...
   --profile shuffle 자동 포함, --remove-orphans + --force-recreate 자동
✅ 4 도구 환원 (2026-05-17): 14 도구 → 4 오픈소스만 (학생 PoC 라이선스 0).
   Keycloak 65 + Wazuh 122 + Nmap 14 + Trivy 11 = 212 함수 (xlsx 자동 진단 212항목과 1:1)

...
| 도구 | 상태 | 자동 항목 수 | 비고 |
|---|---|---|---|
| Wazuh | ✅ | 122 | API + Indexer (REST → JWT). 오픈소스 HIDS 1위 |
| Splunk | ✅ | 15 | Basic Auth + search/jobs/export. 글로벌 SIEM 1위 |
| Elastic SIEM | ⏳ | 0 → 수동 | |
| 기타 | — | 0 → 수동 | |
...
| 도구 | 상태 | 자동 항목 수 | 비고 |
|---|---|---|---|
| CrowdStrike Falcon | ✅ | 15 | OAuth client_credentials. 글로벌 EDR 1위 |
| MS Defender for Endpoint | ✅ | 15 | Entra OAuth scope=securitycenter. M365 통합 |
| 기타 | — | 0 → 수동 | |

#### 클라우드 자세 (CSPM)
| 도구 | 상태 | 자동 항목 수 | 비고 |
|---|---|---|---|
| **AWS Security Hub** | ✅ | 15 | boto3 (IAM Access Key + region). GuardDuty/Inspector findings 집계 |
| **Azure Defender for Cloud** | ✅ | 15 | Entra OAuth → ARM REST. Subscription 단위 |
| GCP Security Command Center | ⏳ | 0 → 수동 | |

#### ZTNA (제로트러스트 네트워크 액세스)
| 도구 | 상태 | 자동 항목 수 | 비고 |
|---|---|---|---|
| **Zscaler** | ✅ | 10 | ZIA/ZPA API key. 글로벌 ZTNA 점유율 상위 |
| **Cloudflare Access** | ✅ | 10 | Zero Trust API token + Account ID |
| 기타 | — | 0 → 수동 | |
...
**현재 자동 진단 합계: base 243 + autodiscover 114 = 357 (14 도구)**.
**xlsx ↔ 매핑 학술 검증 완료**: 자동 진단 212항목 ↔ 우리 매핑 unique item_id 212개 (1:1), 잘못된 매핑 0, 수동 충돌 0, 의도된 다중 매핑(같은 item_id를 IdP 4종/SIEM 2종/EDR 2종/Cloud 2종/ZTNA 2종이 각자의 신호로 측정) 54건. 검증 스크립트: `backend/scripts/validate_checklist_mapping.py`.
...
# (mapping_loader, takes_args) — takes_args=True 인 도구는 (item_id, maturity) 인자 사용
```
도구 추가는 **종료**(2026-05-17 결정). 이후 작업은 매핑 정확성·체크리스트 보완 우선.

#### 카테고리 5종 사용자 선택 ↔ 자동 도구
```python
_IDP_TOOL_OF   = {"keycloak":"keycloak", "entra":"entra", "okta":"okta", "ldap":"ldap"}
_SIEM_TOOL_OF  = {"wazuh":"wazuh", "splunk":"splunk"}
_EDR_TOOL_OF   = {"crowdstrike":"crowdstrike", "defender":"defender"}
_CLOUD_TOOL_OF = {"aws_securityhub":"aws_securityhub", "azure_defender":"azure_defender"}
_ZTNA_TOOL_OF  = {"zscaler":"zscaler", "cloudflare_access":"cloudflare_access"}
```
사용자가 카테고리 1개를 선택해도 그 도구의 매핑만 활성. 미선택/`none` → 해당 카테고리 자동 도구 전부 비활성 → 수동 진단 폴백.
...
#### MS Entra ID (`entra_collector.py`, 570줄)
- 인증: OAuth 2.0 client_credentials (`/oauth2/v2.0/token` → access_token)
- Microsoft Graph: `https://graph.microsoft.com/v1.0/...`
- 토큰 캐시 + 세션 전환 시 무효화
- 20 함수: users, identityProviders, conditionalAccess policies, authenticationMethodsPolicy, directoryRoles, applications, servicePrincipals
- 필요 권한 (Graph App permissions): `Directory.Read.All`, `Policy.Read.All`, `AuditLog.Read.All`

#### Okta (`okta_collector.py`)
- 인증: SSWS API token. `https://{org}.okta.com/api/v1/...`
- 15 함수: 사용자/그룹/MFA factor/sign-on policy/log

#### LDAP/AD (`ldap_collector.py`)
- 인증: ldap3 simple bind. AD/OpenLDAP 듀얼 지원
- 15 함수: 사용자 검색, OU 구조, password policy, account lockout, group membership

#### Splunk (`splunk_collector.py`)
- 인증: Basic Auth 또는 token. `/services/search/jobs/export`
- 15 함수: SPL 기반 alert/threat/correlation 카운트

#### CrowdStrike Falcon (`crowdstrike_collector.py`)
- 인증: OAuth client_credentials `/oauth2/token`
- 15 함수: 디바이스 인벤토리, EDR agent online, detect, behaviors_processed.tactic 검색

#### MS Defender for Endpoint (`defender_collector.py`)
- 인증: Entra OAuth scope=`https://api.securitycenter.microsoft.com/.default`
- 15 함수: machines(onboarded/active), alerts (severity/category filter)

#### AWS Security Hub (`aws_security_hub_collector.py`)
- 인증: IAM Access Key (boto3). region 지정
- 15 함수: findings (SeverityLabel HIGH/CRITICAL + RecordState ACTIVE), Config rule, GuardDuty findings

#### Azure Defender for Cloud (`azure_defender_cloud_collector.py`)
- 인증: Entra OAuth → ARM REST. Subscription ID 단위
- 15 함수: `/providers/Microsoft.Security/alerts`, secureScore, regulatoryCompliance

#### Zscaler (`zscaler_collector.py`)
- 인증: API key. ZIA/ZPA REST
- 10 함수: subnet topology, micro-segmentation rule, TLS inspection ratio

#### Cloudflare Access (`cloudflare_access_collector.py`)
- 인증: API token + Account ID
- 10 함수: Access Application 정책, IdP federation, perimeter model
...
### 도구 fallback (사용자 입력 없을 때) — 14 도구
```
# IdP
KEYCLOAK_URL / KEYCLOAK_ADMIN / KEYCLOAK_ADMIN_PASSWORD
ENTRA_TENANT_ID / ENTRA_CLIENT_ID / ENTRA_CLIENT_SECRET
OKTA_DOMAIN / OKTA_API_TOKEN
LDAP_URL / LDAP_BIND_DN / LDAP_PASSWORD / LDAP_BASE_DN

# SIEM
WAZUH_URL / WAZUH_USER / WAZUH_PASSWORD
SPLUNK_URL / SPLUNK_USER / SPLUNK_PASSWORD (또는 SPLUNK_TOKEN)
...
# ZTNA
ZSCALER_BASE_URL / ZSCALER_API_KEY
...
SHUFFLE_WORKFLOW_KEYCLOAK / _WAZUH / _NMAP / _TRIVY / _ENTRA / _OKTA / _SPLUNK /
_LDAP / _CROWDSTRIKE / _DEFENDER / _AWS_SECURITYHUB / _AZURE_DEFENDER /
_ZSCALER / _CLOUDFLARE_ACCESS
...
- collector 추가/수정 시 `_full_mapping(tool)` 호출해 매핑 수치 확인
- 매핑 docstring 변경 시 **반드시** `docker exec zt-assessment-zt-backend-1 python /app/scripts/validate_checklist_mapping.py` 실행해 xlsx ↔ 매핑 정합성 확인 (누락/잘못/충돌 모두 0건이어야 함)

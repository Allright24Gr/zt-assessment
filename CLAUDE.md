...
학생 프로젝트 — **라이선스 비용 0의 4개 오픈소스 도구만 실제 검증/운영**.
...
| **Trivy** | ✅ | 11 | Apache 2.0 | trivy-wrapper Flask 컨테이너 호출 |

**현재 자동 진단 합계: base 98 + autodiscover 114 = 212 (4 오픈소스 도구)**.
**xlsx ↔ 매핑 학술 검증 완료**: 자동 진단 212항목 ↔ 우리 매핑 unique item_id 212개 (1:1 정합). 잘못된 매핑 0, 수동 충돌 0, 다중 매핑 0. 검증 스크립트: `backend/scripts/validate_checklist_mapping.py`.
...
#### 도구 매핑 (`_TOOL_DISPATCH`, 4 오픈소스 도구)
```python
ALL_TOOLS = (
    "keycloak",  # IdP
    "wazuh",     # SIEM/HIDS
...
### 도구 fallback (사용자 입력 없을 때) — 4 오픈소스 도구
```
# IdP

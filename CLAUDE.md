...
│   ├── collectors/          # 8개 — keycloak/wazuh/nmap/trivy/web_probe/supabase/vercel/railway
...
5. **모든 보호 엔드포인트는 JWT(Bearer) 인증 + 세션·조직 권한 검증.** JWT가 기본(access 8h / refresh 30d)이며 X-Login-Id 헤더는 레거시 호환 폴백으로 공존 — ⚠️ 폴백은 비밀번호·토큰 검증 없이 login_id만으로 통과하므로 운영 배포 전 폴백 제거/비활성 권장.
...
### B-4. 지원 도구 매트릭스 (2026-06-04 기준)

학생 프로젝트 — **8개 진단 도구 중 4개가 라이선스 비용 0의 오픈소스 핵심**(Keycloak/Wazuh/Nmap/Trivy).
나머지 4개는 도구무관 외부 probe(web_probe)와 SaaS 관리 API 3종(Supabase/Vercel/Railway)이며,
web_probe는 공개 probe만 사용하고 Supabase/Vercel/Railway는 고객 자신의 SaaS API 토큰을 사용하므로
**우리 라이선스 비용은 8개 모두 0**이다.
상용 SaaS(Entra/Okta/Splunk/CrowdStrike/Defender/AWS SH/Azure Defender/Zscaler/Cloudflare)는
도입 결정 후 추가 통합 검토.

#### IdP (신원)
| 도구 | 상태 | 자동 항목 수 | 라이선스 | 비고 |
|---|---|---|---|---|
| **Keycloak** | ✅ | 65 | Apache 2.0 | admin REST API token. 대학/공공/금융 일부 |
| 그 외 (사용 안 함 / 기타) | — | 0 → 수동 폴백 | — | 수동 진단 모드 |

#### SIEM / HIDS (보안 정보)
| 도구 | 상태 | 자동 항목 수 | 라이선스 | 비고 |
|---|---|---|---|---|
| **Wazuh** | ✅ | 122 | GPL v2 | API + Indexer (REST → JWT). 오픈소스 HIDS 1위 |
| 그 외 (사용 안 함 / 기타) | — | 0 → 수동 폴백 | — | 수동 진단 모드 |

#### 외부 스캔 (도구 무관)
| 도구 | 상태 | 자동 항목 수 | 라이선스 | 비고 |
|---|---|---|---|---|
| **Nmap** | ✅ | 14 | GPL v2 | nmap-wrapper Flask 컨테이너 호출 |
| **Trivy** | ✅ | 15 | Apache 2.0 | trivy-wrapper Flask 컨테이너 호출 |

#### 도구무관 외부 probe / SaaS 관리 API
| 도구 | 상태 | 자동 항목 수 | 라이선스 | 비고 |
|---|---|---|---|---|
| **web_probe** | ✅ | 24 | — | 공개 probe만 사용 (도구무관 외부 관측) |
| **Supabase** | ✅ | 26 | — | 고객 자신의 SaaS 관리 API 토큰 사용 |
| **Vercel** | ✅ | 10 | — | 고객 자신의 SaaS 관리 API 토큰 사용 |
| **Railway** | ✅ | 8 | — | 고객 자신의 SaaS 관리 API 토큰 사용 |

**현재 자동 진단 합계: 246항목 (8개 진단 도구)**.
도구별 매핑 함수 수: Keycloak 65, Wazuh 122, Nmap 14, Trivy 15, web_probe 24, Supabase 26, Vercel 10, Railway 8 (합 284 매핑; 다중매핑 37건 중복 제외 → unique item_id 246).
**xlsx ↔ 매핑 학술 검증 완료**: 자동 진단 246항목 ↔ 우리 매핑 unique item_id 246개 (1:1 정합). 누락 0, 잘못된 매핑 0, 수동 충돌 0, 다중 매핑 37. 검증 스크립트: `backend/scripts/validate_checklist_mapping.py`.
...
#### 권한
- 모든 보호 엔드포인트: **JWT(Bearer) 검증이 기본** (access 8h / refresh 30d, `get_current_user` 의존성). `X-Login-Id` 헤더는 레거시 호환 폴백으로 공존
- ⚠️ X-Login-Id 폴백은 비밀번호·토큰 검증 없이 login_id만으로 인증을 통과시키므로(예: `X-Login-Id: admin`) **운영 배포 전 폴백 제거/비활성 권장**
- frontend apiFetch가 X-Login-Id를 자동 첨부 (register/login 제외)
...
#### 감사 로그
- 저장소: **`AuthAuditLog` DB 테이블로 운영 중** (해시체인 `prev_hash`/`row_hash`로 변조 탐지)
...
class ProfileSelect(BaseModel):
    idp_type:  Optional[str]  # keycloak | none
    siem_type: Optional[str]  # wazuh    | none
...
class AssessmentRunRequest(BaseModel):
    ...
    scan_mode: str | None,          # "demo" | "live"
```

#### 핵심 함수
- `_resolve_supported_tools(profile_select, requested)` — tool_scope ∩ 사용자 환경
- `_store_session_secrets(session_id, kc, wz)` — 메모리 dict + Lock
- `_pop_session_secrets(session_id)` — 메모리에서 꺼내고 즉시 삭제
- `_mask_creds(extra)` — 응답 마스킹 (keycloak_creds.admin_pass / wazuh_creds.api_pass → "***")
- `_run_collectors(session_id, tools)` — BackgroundTask. `_collector_lock`으로 직렬화. scan_mode 분기.
- `_run_demo_mode(session_id, tools)` — collector 실호출 없이 deterministic fake 결과 생성
...
#### 도구 매핑 (`_TOOL_DISPATCH`, 8개 진단 도구)
```python
ALL_TOOLS = (
    "keycloak",   # IdP (오픈소스 핵심)
    "wazuh",      # SIEM/HIDS (오픈소스 핵심)
    "nmap",       # 외부 스캔 (오픈소스 핵심)
    "trivy",      # 이미지 스캔 (오픈소스 핵심)
    "web_probe",  # 도구무관 외부 probe (공개 probe)
    "supabase",   # SaaS 관리 API (고객 토큰)
    "vercel",     # SaaS 관리 API (고객 토큰)
    "railway",    # SaaS 관리 API (고객 토큰)
)
...
#### 카테고리 선택 ↔ 자동 도구
```python
_IDP_TOOL_OF  = {"keycloak": "keycloak"}   # idp_type=keycloak → keycloak 활성, "none" → 비활성
_SIEM_TOOL_OF = {"wazuh":    "wazuh"}      # siem_type=wazuh   → wazuh 활성,    "none" → 비활성
```
사용자가 `idp_type` 또는 `siem_type` 을 `none` 으로 선택하면 그 카테고리 자동 도구는 비활성 → 수동 진단 폴백. Nmap/Trivy 는 externalScanTools 체크박스로 독립 토글.
...
- 15 함수: 이미지 스캔, CI/CD 스캔 비율, 무결성 검증, SBOM, 의존성 스캔
...
### 헤더
- 보호 엔드포인트: **JWT(Bearer) 인증이 기본** (access 8h / refresh 30d). `X-Login-Id` 헤더는 레거시 호환 폴백으로 공존 (apiFetch 자동 첨부) — ⚠️ 운영 배포 전 폴백 제거/비활성 권장
...
### 도구 fallback (사용자 입력 없을 때) — 8개 진단 도구 중 4 오픈소스 핵심
```
# IdP
...
7. **보호 엔드포인트는 JWT(Bearer) 의존성 + 세션·조직 권한 검증** (X-Login-Id는 레거시 폴백, 운영 전 제거/비활성 권장)
...
**P0 (운영 시작 전 필수)**
1. ~~JWT 세션 토큰화~~ ✅ 완료 (JWT 기본, X-Login-Id 레거시 폴백 → 운영 전 폴백 제거/비활성 권장)
2. 비밀번호 재설정 이메일 흐름
3. ~~audit log DB 테이블화~~ ✅ 완료 (AuthAuditLog 테이블, 해시체인 변조 탐지)

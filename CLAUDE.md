...
DB 테이블 (10개)
Organization, User, DiagnosisSession, Checklist,
CollectedData, Evidence, DiagnosisResult,
MaturityScore, ImprovementGuide, ScoreHistory
API 엔드포인트
...
dev                      통합 테스트 브랜치
feature/backend-skeleton 서진우 담당
feature/keycloak-collector 공나영 담당
feature/wazuh-collector  서정우 담당
feature/nmap-trivy-wrapper 송민희 담당
EC2 서버
IP: 3.35.200.145
OS: Ubuntu 24.04
스펙: t3a.xlarge (4vCPU / 16GB)
주의사항

모든 민감 정보는 .env에서 읽을 것 (하드코딩 금지)
frontend/ 작업자는 backend/ 건드리지 말 것
backend/ 작업자는 frontend/ 건드리지 말 것

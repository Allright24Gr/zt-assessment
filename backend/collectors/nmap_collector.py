...
def set_session_target(target: str | None) -> None:
    """세션별 nmap 스캔 대상을 주입(또는 None으로 해제)."""
    global _current_target
    _current_target = (target or None)


def _get_target() -> str:
    return _current_target or NMAP_TARGET
...
def collect_host_discovery() -> CollectedResult:
    ...
    data, error = _scan_ports({"target_ip": _get_target(), "ports": "1-1024"})
    if error:
...
def collect_port_service_map() -> CollectedResult:
    ...
    data, error = _scan_ports({"target_ip": _get_target(), "ports": "1-65535"})
    if error:
        return _err(item_id, maturity, mk, thr, error, data)
...
def collect_subnet_topology() -> CollectedResult:
    ...
    data, error = _scan_subnets({"network_range": _get_target()})
    if error:
        return _err(item_id, maturity, mk, thr, error, data)
...
def collect_vpn_ports() -> CollectedResult:
    ...
    data, error = _scan_ports({"target_ip": _get_target(), "ports": "500,1194,1723,4500"})
    if error:
        return _err(item_id, maturity, mk, thr, error, data)

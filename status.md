...
│   │   ├── auth.py                # 회원가입/로그인 (PBKDF2-SHA256) + 사용자 프로필
...
│   │
│   └── scripts/                   # 부팅 시 자동 실행
│       ├── migrate_schema.py      # 신규 컬럼 idempotent ALTER (User.login_id/password_hash/profile 등)
│       ├── seed_checklist.py      # xlsx → Checklist 테이블 적재
│       ├── seed_improvement.py    # xlsx → ImprovementGuide 적재
│       ├── seed_demo.py           # (구) 데모_조직 1건 시드 — 보존용
│       └── seed_demo_examples.py  # (신) 세션 전체 wipe + admin/user1 계정 + 예시 8건 시드
│
...
│       └── app/
│           ├── App.tsx
│           ├── routes.tsx         # 8개 페이지 라우팅 (Login, Signup 포함)
│           ├── context/
│           │   └── AuthContext.tsx  # 백엔드 /api/auth/* 호출, localStorage 세션
│           ├── components/
...
│           └── pages/
│               ├── Login.tsx        # 백엔드 /api/auth/login + 회원가입 링크 + admin/user1 데모 버튼
│               ├── Signup.tsx       # 회원가입 폼 (계정 + 진단 프로필 — 진단 프로필 모두 선택)
│               ├── Dashboard.tsx    # 종합 점수/추세/취약 필러/최근 세션 (데모 배지)
│               ├── NewAssessment.tsx# 3단계 진단 신청 폼 + 사용자 프로필 자동 prefill
│               ├── InProgress.tsx   # 자동수집 폴링 + 수동 진단 입력
│               ├── Reporting.tsx    # 4탭 결과 + is_demo 배너
│               ├── History.tsx      # 세션 비교 + 데모 배지
...
| **User** | user_id, org_id, name, email, role, mfa_enabled, **login_id**, **password_hash**, **profile(JSON)** | 담당자/관리자 계정. login_id/password_hash로 인증, profile에 진단 시 prefill용 기본값(부서/연락처/임직원수 등) 저장 |
...
### 4.7 `/api/auth/*` — 회원가입 / 로그인

| 메서드 | 경로 | 기능 |
|---|---|---|
| POST | `/register` | 신규 사용자 + (선택) 조직 + 진단 프로필 등록. PBKDF2-SHA256 200k 라운드 해싱 |
| POST | `/login`    | login_id + password 검증, AuthUser 반환 (org_name/profile 포함) |
| GET  | `/me?login_id=…` | 새로고침 시 최신 user/profile 재조회 (간이 세션) |
| PUT  | `/profile?login_id=…` | 진단 프로필 갱신 (선택 필드 모두 nullable) |
...
1. `/login`에서 데모 계정 클릭(자동 입력) 또는 직접 입력
   - `admin/admin`: 관리자, 전체 조직 이력 조회 가능 (시스템관리 조직)
   - `user1/user1`: 박기웅 (세종대학교) — 일반 사용자, 자기 조직 세션만 조회
2. `/signup`에서 신규 가입 가능. 가입 시 진단 프로필(기관명/부서/연락처/산업군/인프라 유형/임직원 수/서버 수/앱 수/메모)을 함께 저장하면, 신규 진단 폼이 자동 prefill 됨
3. AuthContext: 로그인 성공 시 `/api/auth/login` 결과를 localStorage에 저장. 새로고침 시 `/api/auth/me?login_id=…`로 최신 프로필 재조회
...
docker compose up
  ...
  └─ zt-backend:   entrypoint.sh
        ...
        5. seed_demo_examples.py ── admin/user1 + 박기웅(세종대) + 관리자 예시 4건 + user1 완료 3건/진행 1건 (admin·user1 존재 시 skip, --force로 wipe+재생성)
...
컨테이너 첫 부팅 시 `seed_demo_examples.py`가 실행. admin/user1이 이미 있으면 스킵, 없으면 모든 세션 wipe 후 아래 데이터 생성:

**계정 (PBKDF2-SHA256 해싱)**

| login_id | password | 역할  | 조직 |
|----------|----------|-------|------|
| admin    | admin    | admin | 시스템관리 |
| user1    | user1    | user  | 세종대학교 (담당자: 박기웅, profile prefill 포함) |

**관리자(admin)가 이력 화면에서 보는 예시 4건 (완료)**
...
**user1(박기웅·세종대학교) 자체 이력 — 완료 3건 + 진행 중 1건**

| # | 상태 | 시점 | 메모 |
|---|------|------|------|
| 1 | 완료 | 90일 전 | 1차 진단 — 시작점 측정 (중하) |
| 2 | 완료 | 60일 전 | 2차 진단 — 1분기 개선 후 (중상) |
| 3 | 완료 | 15일 전 | 3차 진단 — 운영 안정화 (중상) |
| 4 | 진행 중 | 2분 전 | 시연용 진행중 진단 — 자동수집 40% |

결과 분포는 4개 PROFILES(`PROFILES[0..3]`)와 `check_id % 10`로 결정 → 재배포해도 매번 동일.

수동으로 다시 wipe·재시드: `docker exec zt-assessment-zt-backend-1 python3 /app/scripts/seed_demo_examples.py --force`

---

## 13. 알려진 한계 (TODO)

- 자체 회원가입/로그인 (PBKDF2-SHA256, 세션 = localStorage) — 운영 시 Keycloak OIDC 연동 권장
- 토큰/세션 기반 인증 미구현 — `/api/auth/me`는 login_id를 query param으로 받는 간이 방식
...
| 파일 | 책임 |
|---|---|
| `main.py` | FastAPI 앱 생성, CORS, 7개 router 등록 (assessment/score/improvement/report/manual/checklist/auth), `/health` |
...
| `routers/auth.py` | 회원가입/로그인/me/profile (PBKDF2-SHA256 200k 라운드, stdlib only — bcrypt 미사용) |
...
| `scripts/seed_demo.py` | (구) 데모_조직 1건 시드 — 보존만, 부팅 시 호출되지 않음 |
| `scripts/seed_demo_examples.py` | (신, 부팅 시 호출) admin/user1 + 박기웅(세종대) + 관리자 예시 4건 + user1 완료 3건/진행 1건. idempotent, --force로 wipe+재생성 |
...
| `app/context/AuthContext.tsx` | 백엔드 /api/auth/* 호출 + localStorage 세션, register/login/refresh 함수 노출 |
...
| `app/pages/Login.tsx` | 백엔드 인증 + admin/user1 데모 버튼 + 회원가입 링크 |
| `app/pages/Signup.tsx` | 계정 생성(필수) + 진단 프로필(선택). 가입 즉시 자동 로그인 |
